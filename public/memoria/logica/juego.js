...
let nivelAnterior = CONFIG.nivelAnterior;
if(nivelAnterior){
    nivelAnterior.addEventListener('click', ()=>{
        window.location.href = CONFIG.urlNivelAnterior;
    });
}
...
let puntos = 0;
const PUNTOS_POR_ACIERTO = CONFIG.puntosPorAcierto; //variable de configuración
let segundos = CONFIG.segundos; //variable de configuración
let intervalo = null;
        ...
        if(tiempo){
            tiempo.textContent = segundos;
                if(segundos === 0){
                    ...
                    segundos = CONFIG.segundos; // REVISAR SI SE PUEDE AGREGAR A CONFIG
                    perdiste();
                }
            }
        }
